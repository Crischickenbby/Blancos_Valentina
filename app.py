...
def apartado():
    ...
    try:
        ...
        cur.execute('''SELECT l."ID_Layaway", l."Name", l."Last_Name", l."Phone", l."Date", l."Due_Date", 
                              l."Pending_Amount", p."Name" AS product_name, s."Status", p."Price"
                       FROM "Layaway" l
...
def crear_apartado():
    try:
        ...
        # 7. Insertar el primer pago en Layaway_Payments solo si hay abono inicial
        if abono_inicial > 0:
            cur.execute(''' 
        ...
        # 10. Registrar en caja solo si hay abono inicial
        if abono_inicial > 0:
            ...
            # Calcular nuevo saldo si aplica (para efectivo)
            nuevo_saldo = saldo_actual
            if id_metodo_pago == 1:  # Efectivo
                nuevo_saldo += abono_inicial

            # Insertar el movimiento de caja
            cur.execute(''' 
                INSERT INTO "Cash"
...
def historial_pagos_apartado():
    ...
    if not id_layaway:
        return jsonify({'error': 'ID de apartado requerido'}), 400
    
    conn = get_db_connection()
    cur = conn.cursor()
    
    try:
        # Verificar si existen pagos para este apartado
        cur.execute('SELECT COUNT(*) FROM "Layaway_Payments" WHERE "ID_Layaway" = %s', (id_layaway,))
        count_pagos = cur.fetchone()[0]
        ...
        # Consulta simplificada para obtener solo los pagos reales del apartado
        query = '''
            SELECT 
                lp."Amount_Paid" as monto,
                lp."Payment_Date" as fecha,
                'Efectivo' as metodo_pago,
                lp."ID_Layaway_Payment" as id_pago
            FROM "Layaway_Payments" lp
            WHERE lp."ID_Layaway" = %s
            ORDER BY lp."Payment_Date" ASC, lp."ID_Layaway_Payment" ASC
        '''
        
        cur.execute(query, (id_layaway,))
        pagos = cur.fetchall()
        ...
        for pago in pagos:
            historial.append({
                'monto': float(pago[0]) if pago[0] else 0,
                'fecha': pago[1].strftime('%d/%m/%Y %H:%M') if pago[1] else 'Fecha no disponible',
                'metodo_pago': pago[2],
                'id_pago': pago[3]
            })
...
def realizar_pago():
    ...
    try:
        ...
        # 1. Convertir monto a float para operaciones (viene como string del JSON)
        monto_float = float(monto)

        # 2. Obtener el total de pagos previos
        cur.execute('''
            SELECT COALESCE(SUM("Amount_Paid"), 0) FROM "Layaway_Payments"
            WHERE "ID_Layaway" = %s
        ''', (id_layaway,))
        total_pagado = float(cur.fetchone()[0])

        # 3. Obtener el monto original del producto y el usuario
        cur.execute('''
        ...
        nuevo_saldo = saldo_pendiente - monto_float

        fecha_pago = datetime.now()

        # 4. Insertar el nuevo pago (sin ID_Payment_Method ya que la columna no existe)
        cur.execute('''
            INSERT INTO "Layaway_Payments" ("ID_Layaway", "Amount_Paid", "Payment_Date")
            VALUES (%s, %s, %s)
        ''', (id_layaway, monto_float, fecha_pago))
        ...
        # 6. Registrar en caja (todos los pagos se registran, pero solo efectivo cambia el saldo)
        cur.execute('SELECT "Current_Effective" FROM "Cash" ORDER BY "ID_Cash" DESC LIMIT 1')
        saldo_result = cur.fetchone()
        saldo_actual = float(saldo_result[0]) if saldo_result else 0.0
        
        # Solo el efectivo afecta el saldo de caja
        if metodo_pago == 1:  # Efectivo
            nuevo_saldo_caja = saldo_actual + monto_float
        else:  # Tarjeta o transferencia
            nuevo_saldo_caja = saldo_actual  # El saldo no cambia

        cur.execute('''
            INSERT INTO "Cash"
            ("Amount", "Current_Effective", "ID_Sale", "ID_Transaction_Type", "ID_Payment_Method", "ID_User", "Date")
            VALUES (%s, %s, %s, 1, %s, %s, %s)
        ''', (monto_float, nuevo_saldo_caja, id_sale, metodo_pago, id_usuario, fecha_pago))
...
def cancelar_apartado():
    data = request.get_json()
    id_layaway = data.get('id_layaway')
    metodo_reembolso = data.get('metodo_reembolso', 1)  # 1 = efectivo, 2 = transferencia, 3 = tarjeta
    ...
    try:
        ...
        cur.execute('''
            ...
            WHERE "ID_Layaway" = %s
        ''', (id_layaway,))
        total_abonado = float(cur.fetchone()[0] or 0)

        # 7. Solo procesar reembolso si hay dinero que devolver
        if total_abonado > 0:
            # Obtener el último saldo de efectivo
            cur.execute('''
            ...
            # 8. Calcular nuevo saldo y monto según método de reembolso
            if metodo_reembolso == 1:  # Efectivo - se reduce del saldo de caja
                nuevo_saldo = saldo_actual - total_abonado
                monto_egreso = total_abonado  # Positivo para egresos
            else:  # Transferencia o Tarjeta - no afecta saldo de efectivo
                nuevo_saldo = saldo_actual
                monto_egreso = total_abonado  # Se registra el monto pero no afecta efectivo

            # 9. Registrar egreso en caja con el método seleccionado
            cur.execute('''
                INSERT INTO "Cash"
                ("Amount", "Current_Effective", "ID_Sale", "ID_Transaction_Type", "ID_Payment_Method", "ID_User", "Date")
                VALUES (%s, %s, %s, 2, %s, %s, %s)
            ''', (monto_egreso, nuevo_saldo, id_sale, metodo_reembolso, id_usuario, fecha_cancelacion))
            
        else:
            pass  # No hay dinero que devolver, no se registra movimiento en caja        conn.commit()
        cur.close()
        conn.close()
